      ...
      {isBookingOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(255, 242, 230, 0.95)', zIndex: 4000, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px', overflowY: 'auto' }}>
          <div style={{ backgroundColor: 'var(--color-sunset)', padding: '20px', borderRadius: '30px', border: '1.5px solid var(--color-pink)', width: '100%', maxWidth: '350px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <button onClick={() => { setIsBookingOpen(false); setActiveTab('Todos'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} style={{ backgroundColor: 'var(--color-pink)', color: 'var(--color-crema)', border: 'none', borderRadius: '10px', padding: '12px', fontWeight: 'bold', cursor: 'pointer' }}>Confirmar Reserva</button>
            <button onClick={() => { setIsBookingOpen(false); setActiveTab('Todos'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} style={{ background: 'none', border: 'none', color: 'var(--color-orange)', cursor: 'pointer' }}>Cerrar</button>
          </div>
        </div>
      )}
      ...
      {isCartOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(255, 242, 230, 0.95)', zIndex: 3000, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px', overflowY: 'auto' }}>
          <div style={{ backgroundColor: 'var(--color-sunset)', padding: '25px', borderRadius: '30px', border: '1.5px solid var(--color-pink)', width: '100%', maxWidth: '350px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              {productosEnCarrito.length === 0 ? <p style={{ color: 'var(--color-mocha)', textAlign: 'center' }}>Carrito vacío</p> : productosEnCarrito.map(item => (
                <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--color-crema)', paddingBottom: '10px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', width: '50%' }}>
                    <span style={{ color: 'var(--color-mocha)', fontSize: '13px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.nombre}</span>
                    <span style={{ color: 'var(--color-orange)', fontSize: '12px' }}>${item.precio * quantities[item.id]}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <button onClick={() => updateQuantity(item.id, -1)} style={{ backgroundColor: 'var(--color-crema)', border: 'none', color: 'var(--color-mocha)', borderRadius: '5px', width: '25px', height: '25px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>-</button>
                    <span style={{ color: 'var(--color-mocha)', fontWeight: 'bold', width: '15px', textAlign: 'center', fontSize: '14px' }}>{quantities[item.id]}</span>
                    <button onClick={() => updateQuantity(item.id, 1)} style={{ backgroundColor: 'var(--color-crema)', border: 'none', color: 'var(--color-mocha)', borderRadius: '5px', width: '25px', height: '25px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
                  </div>
                </div>
              ))}
            ...
            <button onClick={() => setIsCartOpen(false)} style={{ backgroundColor: 'var(--color-pink)', color: 'var(--color-crema)', border: 'none', borderRadius: '10px', padding: '12px', fontWeight: 'bold', marginTop: '10px', cursor: 'pointer' }}>Confirmar Pedido</button>
